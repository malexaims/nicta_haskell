...
(|||) ::
  ...
  -> Parser a
  -> Parser a
(|||) (P p1) (P p2) = P (\x -> if isErrorResult $ p1 x
                                then p2 x
                                else p1 x)
...
-- list recursively calls until list1 fails, then the list is completed by appending Nil
...
sequenceParser ::
  List (Parser a)
  -> Parser (List a)
sequenceParser l = foldRight (lift2 (:.)) (pure Nil) l
