...
instance Traversable ListZipper where
  -- lift3 ::
  --   Applicative f =>
  --   (a -> b -> c -> d) This is ListZipper's type: three inputs that result in a ListZipper
  --   -> f a
  --   -> f b
  --   -> f c
  --   -> f d
  traverse f (ListZipper l c r) = lift3 ListZipper (traverse f l) (f c) (traverse f r)
...
instance Traversable MaybeListZipper where
  traverse _ IsNotZ = lift0 IsNotZ
  traverse f (IsZ lz) = lift1 IsZ $ traverse f lz
